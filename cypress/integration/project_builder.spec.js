...
describe('user must complete form/add skills before creating project', () => {
  it('cannot add to the project list', () => {
    rootPage.goToRoot();
    rootPage.projectCreateButton().click();
    rootPage.projectsList().should('have.length', 0);
    rootPage.projectBuilderError().should('be.visible');
  });
});
